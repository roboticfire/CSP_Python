...
def text_on_image(image, text, size, border, r,g,b):
    ...
    write.text( ((width-w)/2, (height*9/10)-h ),lines,(r,g,b),font=font)
...
def user_input():
    ...
    print("\n------------------------------------------------------------")
    print("============= Shuttergram Image Maniplations ==============")
    print("========= Developers: Joey Huang and Edmund Feng ==========")
    print("------------------------------------------------------------")
    print("\n===================== Border =====================")
    ...
    while not done:
        ...
        if borderChoice == "a":
            ...
            # Prints an error for the user if value was not a or b
            print("Input is not a valid option, please enter in A or B")
    done = False
    print("Example: cyan")
    color1 = raw_input("Color 1 >> ")
    color2 = raw_input("Color 2 >> ")
    if borderChoice == "a":
         # Assigns a placeholder in order to fullfill all parameters (not used)
        color3 = "black"
    if borderChoice == "b":
    ...
    print("with 0 being no tint and")
    print("255 being only able to see the tint.")
    print("***Enter in 0 for no tint.***")
    print("Range: 0-255")
    print("Recommended: 75-100")
    while not done:
        try:
            alphaImg = int(raw_input("Alpha (transparent) Value >> "))
            # Checks if integer is able to fit within possible alpha values
            if type(alphaImg) == int and alphaImg<=255 and alphaImg>=0:
                # Ends while loop
                done = True
            else:
                print("Please enter an integer between 0 and 255.\n")
        except ValueError: # Runs this if it can't get a int value from input
            print("Please enter an integer between 0 and 255.")
    
    # Runs if tint is not 0
